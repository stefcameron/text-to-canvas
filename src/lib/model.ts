 ...
 *
 * - `clip`: Render only the words that fit within the configured {@link DrawTextConfig.width}.
 *
 *     🔺 This is an optimization on the `none` option where only the words that will fit within
 *     the configured rendering {@link DrawTextConfig.width} will have `metrics`.
 *
 * - `wrap`: Wrap words at whitespace boundaries within the configured text rendering
 *     {@link DrawTextConfig.width}.
 */
export type TextWrap = 'none' | 'clip' | 'wrap';
